...
#define TEST 6
string run_strategy(int strategy, int player, int depth, int iteration, int order, string board)
{
    ...
    {
        return mix_depth(player, depth, iteration, order, board);
    }
...
int main()
{
    ...
    int strategy[2], input_depth[2], input_iteration[2], color[2], input_order[2];
    ...
    for (int i = 0; i < 2; i++)
    {
        cout << "Please select " << order[i] << " strategy, alpha beta pruning...1/monte carlo...2/mixdepth...3/mixheu...4/history heuristic...5/only alpha...6: ";
        cin >> strategy[i];
        ...
        else if (strategy[i] == MIX_DEPTH)
        {
            cout << "Please enter depth: ";
            cin >> input_depth[i];
            cout << "Please enter iteration: ";
            cin >> input_iteration[i];
            cout << "minmax first(0) monte carlo first(1)";
            cin >> input_order[i];
        }
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        {
            strategy_detail[i] = to_string(input_depth[i]) + "," + to_string(input_iteration[i])+ ","+to_string(input_order[i]);
        }
        else if (strategy[i] == MIX_HEURISTIC)
        {
    ...
    for (int game = 1; game <= 100; game++)
    {
        ...
        while (same != 2)
        {
            ...
            board_str = run_strategy(strategy[index], color[index], input_depth[index], input_iteration[index], input_order[index], board_str);
